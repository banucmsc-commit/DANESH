...
const technologies = [
    ...
    { name: 'Mastercam', description: 'For CNC process automation.', image: 'https://www.mastercam.com/wp-content/uploads/2020/05/Mastercam-Logo.png' },
    ...
    { name: 'Hydro Testing', description: 'For final performance validation.', image: 'https://img.icons8.com/ios-filled/50/000000/test-tube.png' },
];
                    ...
                    <div className="max-w-4xl mx-auto">
                        <ul className="space-y-4">
                            {technologies.map((tech, index) => (
                                <li key={index} className={`bg-white p-6 rounded-lg shadow-sm flex items-center space-x-6 hover:shadow-lg hover:scale-105 transition-all duration-300 animate-fade-in ${tech.name === 'SolidWorks' ? 'solidworks-highlight' : ''} ${tech.name === 'Mastercam' ? 'mastercam-highlight' : ''}`} style={{ animationDelay: `${0.2 + index * 0.15}s` }}>
                                    <div className={`flex-shrink-0 h-12 w-12 rounded-full bg-brand-blue flex items-center justify-center text-white font-bold text-2xl animate-spin-in overflow-hidden ${tech.name === 'SolidWorks' ? 'solidworks-icon' : ''} ${tech.name === 'Mastercam' ? 'mastercam-icon' : ''}`} style={{ animationDelay: `${0.4 + index * 0.15}s` }}>
                                        {tech.image ? <img src={tech.image} alt={tech.name} className="h-8 w-8 object-contain" /> : index + 1}
                                    </div>
                                    <div>
                                        <h3 className={`text-xl font-semibold text-brand-blue hover:text-brand-dark transition-colors duration-300 ${tech.name === 'SolidWorks' ? 'solidworks-title' : ''} ${tech.name === 'Mastercam' ? 'mastercam-title' : ''}`}>{tech.name}</h3>
